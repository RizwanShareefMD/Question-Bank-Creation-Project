...
async function extractText(file) {
  const dataBuffer = new PDFParse({ url: file.path });
  if (file.mimetype === "application/pdf") {
    // Use the imported variable name consistently
    const data = await dataBuffer.getText();
    return data.text;
  } else {
...
app.post("/upload-and-generate", upload.single("file"), async (req, res) => {
  try {
    ...
    let jsonText = (await result.response)
      .text()
      .replace(/\`\`\`json|\`\`\`/g, "")
      .trim();
